...
export function useMapViewState({
  ...
  // Detect device changes (type, size, or significant zoom changes)
  ...
  // Update viewState based on device changes and user interactions
  useEffect(() => {
    const { isDeviceChange, previousZoom } = deviceChangeInfo;

    // Reset user interaction flag on device change to allow auto-positioning
