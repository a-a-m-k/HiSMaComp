...
export const useNavigationControlAccessibility = (
  ...
  useEffect(() => {
    ...
    const ensureNavigationControlAccessible = () => {
      try {
        // Scope to container if available, otherwise use document-wide query
        ...
        if (navControls.length > 0) {
          navControls.forEach(button => {
            ...
            // Remove native tooltips (title attribute)
    ...
    // Try to set immediately, then observe for dynamic updates
    ...
    // Observe the map container for changes (NavigationControl is rendered dynamically)
    // Scoping to container is more efficient than observing entire document.body
    try {
      const mapContainer =
