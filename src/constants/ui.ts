...
export const NAVIGATION_CONTROL_STYLES = {
  ...
  /**
   * Offset to compensate for MapLibre's default bottom positioning.
   * MapLibre defaults to ~12px from bottom, we want 8px (theme.spacing(1)).
   * Offset: 12px - 8px = 4px (theme.spacing(0.5))
   */
  BOTTOM_OFFSET: 1, // theme.spacing(0.5) = 4px offset
  /**
   * Button sizes to match timeline height at each breakpoint.
   * Navigation control has 3 buttons (zoom in, zoom out, compass/rotate).
   * MapLibre buttons have 1px borders between them (2 borders total = 2px).
   * Calculation: (timeline height - borders) / 3 = button size
   * For desktop: (100px - 2px) / 3 = 32.67px, but we need exact 100px total
   * So: 3 buttons × button_size + 2 borders = 100px
   * Let button_size = (100 - 2) / 3 = 32.67px ≈ 33px, but that gives 101px total
   * Better: Use 32px buttons + 2px borders = 98px, but we need 100px
   * Solution: Make buttons 33px each: 3 × 33 + 2 = 101px (close) or adjust to fit exactly
   * Actually, borders are part of button box model, so: button_height × 3 = 100px
   * button_height = 100 / 3 = 33.33px. Use 33px for buttons, total = 99px + borders
   * Or: Use 34px buttons: 3 × 34 = 102px (too tall)
   * Best: Use 33px buttons, accept 99-101px total, container exactly 100px with overflow visible for tooltips
   */
  BUTTON_SIZES: {
    MOBILE: Math.floor((TIMELINE_HEIGHTS.MOBILE - 2) / 3), // (87 - 2) / 3 = 28px
    TABLET: Math.floor((TIMELINE_HEIGHTS.TABLET - 2) / 3), // (120 - 2) / 3 = 39px
    DESKTOP: Math.round((TIMELINE_HEIGHTS.DESKTOP - 2) / 3), // (100 - 2) / 3 = 32.67px → 33px
  },
  /**
